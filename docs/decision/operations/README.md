...
| `RUN-JEV-INCIDENT-EVIDENCE-v1` | Incident-evidence preservation | Any declared incident that needs decision traces, receipts, review or job records as evidence; the incident commander owns the response and names an evidence custodian. | Place the affected traces, receipts, review and job records under legal hold before any deletion or TTL sweep, record the custodian, and export only through `sanitizedTelemetryExport()` after a canary scan. | Verify hold and tombstone state, confirm the sanitized export contains no protected canary or provider request ID, attach only sanitized evidence to the incident record, and release the hold only on the incident owner's written closure. |
