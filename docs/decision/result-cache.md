...
Telemetry reports only `hit`, `miss`, `bypass`, `stale`, `invalidation`, and `single-flight` with an opaque operation ID. Semantic keys, input-derived digests, subject identities, and entry IDs are not telemetry fields. Savings account for avoided work only; original provider usage remains attached to original evidence and is never counted again as new usage.
