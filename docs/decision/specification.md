...
`v1alpha2` additionally permits immutable `primitive-policy` acceptance. The policy has a semantic version, an explicit non-empty allow-list of compatible uncertainty profiles, explicit `first-match` precedence, ordered rules, and separate default, missing-evidence, invalid-evidence, tie, and declared-fallback routes. Missing profile metadata follows the missing-evidence route; a profile outside the allow-list follows the invalid-evidence route. This prevents a threshold authored for Jev provider distributions from being silently applied to an LLM self-report or any other uncertainty semantics. Conditions compare named probability-normalized metrics in basis points: Noul yes probability; Choice selected probability, native confidence, top-two margin, normalized entropy and concentration; Score normalized expected value, native confidence, dispersion and concentration; and separately pinned calibrated risk. Raw probability, native confidence, provider distribution, derived statistics, and calibrated values remain distinct receipt evidence with provenance. The comparison projection retains eight decimal places of a basis point to remove binary floating-point representation noise; the raw statistic is never replaced. Policy authors must treat this as a quantized comparison: values less than half of the retained unit apart can compare identically at a boundary. Do not encode distinctions finer than the projection; select a wider review band when such distinctions matter. A policy cannot use a metric from another primitive. Choice may require explicit authored options such as `none` or `other`; validation fails rather than inventing one. Score expected values are normalized only for comparison and the original fractional value is retained without rounding.
...
6. Apply target acceptance. Legacy thresholds retain their existing failure reasons. Primitive policies first require the observation's declared uncertainty profile to be on the policy allow-list, derive only metrics valid for the declared primitive, apply exact inclusive/exclusive boundaries to the documented quantized basis-point projection, and use ordered first-match precedence for intentionally overlapping gray bands. Missing, invalid, tied, or calibration-incompatible evidence follows its explicit non-action route. Acceptance returns routing evidence only and has no action side effect. Receipt replay returns that evidence verbatim without adapter redispatch; a surrounding dispatcher must separately interpret any `act`, `review`, `reject`, or `fallback` disposition. On configured eligible reason, the dispatcher may try the next target with the same definition and input.
...
(including model and credential reference), authorized egress policy, trusted
host-policy identity, deadline,
